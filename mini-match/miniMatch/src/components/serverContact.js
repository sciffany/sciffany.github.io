...
export async function handleEndGame(scene, messageSetter) {
  ...
  // const myObj = { done: Math.floor(Math.random() * 3).toString() }
...
function messageBox(scene) {
  ...
  const textSprite = scene.add
    .text(70, Y_OFFSET - 50, "Contacting server..", serverStyle)
    .setDataEnabled()
...
function encodeDataString(datastring) {
  ...
  for (name in datastring) {
    urlEncodedDataPairs.push(encodeURIComponent(name) + "=" + encodeURIComponent(datastring[name]))
  }
